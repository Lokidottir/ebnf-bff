...
processArgs args
    ...
    | not . prelargs grammarArg $ args =
        ...
        Text.EBNF.Helper.die "error: no grammar was provided (--grammar|-g=<filename>)"
    | (not . prelargs primArgs $ args) &&
      (not . eleml ebnfastArgs $ args) =
        -- there is no primary rule and we're not outputting the EBNF AST
        Text.EBNF.Helper.die "error: no primary rule provided (--primary-rule|-p=<rule>)"
    | (not . eleml sourceArgs $ args) &&
      (not . eleml ebnfastArgs $ args) =
          -- there are no source files and we're not outputting the EBNF AST
        Text.EBNF.Helper.die "error: no source files provided (--source-files|-s ...)"
    ...
    | not . (`elem` formats)
          . getArgData formatArgs $ args =
             Text.EBNF.Helper.die "error: format not supported (json|plaintext|xml)"
    {-
...
main' args = do
    ...
    let parserf gr fname fc =
                      case parse ((fromJust $ lookupGrammar primaryRule gr) gr) fname fc of
                          Left err -> (Text.EBNF.Helper.die . show $ err) >> return ""
    ...
    case parse syntax grammarFile grammarContent of
        Left err -> Text.EBNF.Helper.die . show $ err
        Right st ->
            {-
        ...
        (ifM (doesFileExist h)
            ...
            (ifM (doesDirectoryExist h)
                (do
                    ...
                    sp <- pollSourcePaths t
                    return (sp' ++ sp))
                (Text.EBNF.Helper.die ("error: '" ++ h ++ "' is not a file or directory") >> return [])))
